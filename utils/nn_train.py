...
def train_model_multi_feature(model, n_epochs, data_dict, scaler_y, device, SEED = 42):
    """
    Train a model for time series forecasting, optionally using a validation set.
...
def eval_model_on_test_set_multi_feature(model, results_dict, data_dict, scaler_y, device):
    ...
    def predict_and_calculate_metrics(X_sc, y, y_sc):
        ...
        y_pred_inv = torch.Tensor(scaler_y.inverse_transform(y_pred.cpu().detach().numpy()))

        # RMSE and R^2 for unscaled data
        rmse = np.sqrt(loss_fn(y_pred_inv, y.cpu()).item())
        r2 = r2_score(y.cpu().detach().numpy(), y_pred_inv.cpu().detach().numpy())

        # RMSE and R^2 for scaled data
        rmse_sc = np.sqrt(loss_fn(y_pred.detach(), y_sc.to(device)).item())
        r2_sc = r2_score(y_sc.cpu().detach().numpy(), y_pred.cpu().detach().numpy())

        return y_pred_inv.cpu(), rmse, r2, rmse_sc, r2_sc

    # Predict and calculate metrics for test set
    y_test_pred_inv, test_rmse, test_r2, test_rmse_sc, test_r2_sc = predict_and_calculate_metrics(
        data_dict["X_test_sc"], data_dict["y_test"], data_dict["y_test_sc"]
    )

    # Predict and calculate metrics for validation set
    y_val_pred_inv, val_rmse, val_r2, val_rmse_sc, val_r2_sc = predict_and_calculate_metrics(
