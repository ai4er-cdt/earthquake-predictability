...
def record_metrics(model, data, model_dir):
    ...
    """
    score_dict = {}
    score_dict["user"] = username  # Assuming 'username' is defined somewhere
    score_dict["r2"] = r2_score(data["y_test"], data["y_pred"])  # Calculate R^2 score
    score_dict["mae"] = mean_absolute_error(data["y_test"], data["y_pred"])  # Calculate MAE
    ...
    try:
        ...
        df = pd.DataFrame()  # If file doesn't exist or error reading, create an empty DataFrame

    new_df = pd.DataFrame([score_dict])  # Create a DataFrame from the score dictionary
    combined_df = pd.concat([df, new_df], ignore_index=True)  # Concatenate existing DataFrame with new one
    sorted_df = combined_df.sort_values(by="r2", ascending=False)  # Sort DataFrame by R^2 score
    sorted_df = sorted_df.drop_duplicates(
        subset="model_dir", keep="last"
    ).reset_index(drop=True)  # Remove duplicate models based on model directory
    sorted_df.to_csv(csv_file_path, index=False)  # Write updated DataFrame to CSV
    rank = sorted_df.loc[sorted_df["model_dir"] == model_dir].index[0]  # Find the rank of the current model
